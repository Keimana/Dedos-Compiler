    def tokenize(self, input_string):
        ...
        if not input_string.startswith("~{"):
            raise ValueError(f"Lexical Error: Input must start with '~{{' at position 0.")
        ...
        while i < length:
            ...
            # Handle reserved words (e.g., 'abort', 'and')
            if input_string[i:i+5] == "abort":
                tokens.append(("RESERVED_WORD", "abort"))
                i += 5  # Move past 'abort'
                continue
            elif input_string[i:i+3] == "and":
                tokens.append(("RESERVED_WORD", "and"))
                i += 3  # Move past 'and'
                continue

            # Handle open block "~{"
            if i + 1 < length and input_string[i:i+2] == "~{":
                if block_stack:  # Ensure no block is already open
                    raise ValueError(f"Lexical Error: Unexpected opening block '~{{' at position {i}. A block has already been opened.")
                
                tokens.append(("~{", "~{"))
                block_stack.append("~{")  # Push the opening block onto the stack
                i += 2  # Move past the "~{"
                continue  # Continue to the next character after "~{"

            # Handle close block "}~"
            elif i + 1 < length and input_string[i:i+2] == "}~":
                if not block_stack:  # Ensure a block was previously opened
                    raise ValueError(f"Lexical Error: Unexpected closing block '}}~' at position {i}. Block not opened.")
                
                block_stack.pop()  # Pop the last opened block (matching "~{")
                tokens.append(("}~", "}~"))
                i += 2  # Move past the "}~"
                
                # After the "}~", classify anything after it as UNKNOWN
                while i < length:
                    start = i
                    # Tokenize as UNKNOWN for any remaining characters
                    tokens.append(("UNKNOWN", input_string[start:i+1]))
                    ...
                    if next_char == "(":
                        ...
                        if i < length and input_string[i] == ")":
                            ...
                            if i < length and input_string[i] == "{":
                                ...
                                while i < length and input_string[i] != "}":
                                    # Process each line or token inside '{ }'
                                    start = i
                                    while i < length and input_string[i] != "\n" and input_string[i] != "}":
                                        i += 1
                                    if start < i:
                                        tokens.append(("line", input_string[start:i].strip()))
                                ...
                                raise ValueError(f"Lexical Error: Missing '{{' after 're' at position {i}")
                        else:
                            raise ValueError(f"Lexical Error: Missing closing ')' for 're' at position {i}")
            ...
            # Handle 'load' token
                ...
                # Handle '{' after 'load'
                    ...
                    while i < length and input_string[i] != "}":
                        ...
                        # Parse valid tokens inside '{ }'
                        while i < length and input_string[i] != "}" and input_string[i] != "\n":
                            i += 1

                        # Add content inside '{ }' as a line or token
                        if start < i:
                            tokens.append(("line", input_string[start:i].strip()))
                    ...
                    if i < length and input_string[i] == "}":
                        ...
                        raise ValueError(f"Lexical Error: Missing closing '}}' for 'load' at position {i}")
                ...
                # Handle invalid characters after 'load'
                    ...
                    raise ValueError(f"Lexical Error: 'load' must be followed by space, '{{', or valid character at position {j}")
            ...
            # Handle single quotes for character literals (enclosed in single quotes)
                ...
                while i < length and input_string[i] != "'":
                    i += 1
                if i < length:
                    chat_literal_content = input_string[start:i]
                    if len(chat_literal_content) == 1 and chat_literal_content.isalnum():
                        tokens.append(("chat_literal", chat_literal_content))
                    else:
                        raise ValueError(f"Lexical Error: Invalid character literal '{chat_literal_content}' at position {start}")
                    i += 1
                else:
