...
class Semantic:
    ...
    def parameter(self):
        try:
            ...
            param_str = param_str.replace("\n", "\\n").replace('True', 'pos').replace('False', 'neg').replace('-', '_')
            try:
                try:
                    param_tuple = (eval(param_str.replace("_", "-"), None, self.tool))  # Try to evaluate the param_str
                except:
                    printerr = []
                    parama = eval(param_str.replace("_","-"), None, self.tool)
                    for item in parama:
                        item = str(item)
                        try:
                            ...
                            try:
                                a = eval(item.replace("_","-"), None, self.tool)
            ...
            param_fixed = param_str.replace('True', 'pos').replace('False', 'neg').replace('-', '_')
            return param_fixed
        except Exception as e:
            a = str(e)
    ...
    def loop_condition(self):
        try:
            ...
            if cond_result == "pos":
            ...
            else:  # if "loop if" is neg then go to loop reload or loop load
                ...
                while self.c1 != '<loop condition->' and self.c1 != '<loop else+>':
                    if self.c1 != '<loop elif+>':
                        while self.c1 != '<condition+>':
                            self.next()
                        cond_result = self.condition()
                        if cond_result == "pos" and ctr == 0: # if condition is true in reload then run its body and check if there is already body run before this
                ...
                if self.c1 == '<loop else+>' and ctr == 0: # if loop if and loop reload is false then run loop else, also check if a condition body is already ran
                    if cond_result == "neg":
                        while self.c1 != '<loop condition->':
                            if self.c1 == '<output statement+>':
                                self.output_statement()
                    ...
                    if idtype == "array": #if id is array
                        ...
                        try:
                            if datatype == "inst" or datatype == "flank":
                                value1 = eval(value1.replace("_", "-"), None, self.tool)
                            else:
    ...
    def func_loop_condition(self):
        try:
            ...
            if cond_result == "pos":
                ...
                if self.c11 == '<function loop else+>' and ctr == 0:  # if loop if and loop reload is false then run loop else, also check if a condition body is already ran
                    if cond_result == "neg":
                        while self.c11 != '<function loop condition->':
                            if self.c11 == '<output statement+>':
                                self.func_output()
