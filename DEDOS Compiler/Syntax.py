class DEDOSParser:
    ...
    def Terminal_id(self):  # <id>
        ...
        if 'IDENTIFIER' in self.currentkeys:
            ...
            if (self.currentkeys in ['[', '=', 'plant', 're', 'force', 'watch', 'defuse', '~', 'globe', 'inst',
                ...
                self.SyntaxErrors.append(
                    f'LINE #{self.lineCounter} : Agent, Unexpected ⏵ "{self.currentvalues}" \n\nExpected ⏵ "=", "+", "-", "*", "/", "%", "<", ">", "<=", ">=", "==", "!=", "and", "or", "COMMA""')  # put error in a list for viewing in GUI
        else:
    ...
    def Terminal_math_expression(self):
        ...
        if self.currentkeys == '(':
            ...
            print("SYNTAX ERROR 16.6: Unexpected", self.currentvalues, self.lineCounter)
            self.SyntaxErrors.append(
    ...
    def Terminal_number_value(self):
        if self.currentkeys in ['FLANKLIT', 'INSTLIT']:
            ...
            if self.currentkeys in ['+', '-', '*', '/', '%', ')', 'plant', 're', 'force', 'watch', 'defuse',
                ...
                self.SyntaxErrors.append(
                    f'LINE #{self.lineCounter} : Agent, Unexpected ⏵ "{self.currentvalues}" \n\nExpected ⏵ "+", "-", "*", "/", "%", "<", ">", "<=", ">=", "==", "!=", "and", "or", "COMMA"')  # put error in a list for viewing in GUI:  # put error in a list for viewing in GUI:
        else:
    ...
    def GetNextTerminal(self):  # <program> and <program content>
        ...
        while self.position < len(self.Terminals) and ctr != len(self.Terminals) and terminator != 0:
            if self.currentkeys == '~':
                ...
                self.SyntaxErrors.append(
                    f'LINE #{self.lineCounter} : Agent, Unexpected ⏵ "{self.currentvalues}"')  # put error in a list for viewing in GUI
                break
            ctr += 1
        self.Terminal_Prog_end()
        if len(self.SyntaxErrors) == 0:
            print("SYNTAX COMPILE SUCCESSFUL", self.lineCounter)
            self.SyntaxErrors.append("SYNTAX COMPILE SUCCESSFULLY AGENT!")  # put in a list for viewing in GUI
